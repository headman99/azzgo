...
export class Tab1Page implements OnInit {
  ...
  ngOnInit() {
    this.limit = 30;
    this.usersRequest = new CometChat.ConversationsRequestBuilder()
    .setLimit(50).setConversationType('user')
    .build();
    
  //  new CometChat.UsersRequestBuilder().setLimit(this.limit).build();
    this.getUserList();
    ...
    userList => {
      if (userList.length > 0) {
       ...
       /* CometChat.getUnreadMessageCountForAllUsers().then(array => {
            ...
            if (unread.length > 0) {
                unread.map(uid => {
                    const index = userList.map(u=>u.conversationWith).findIndex(user => user.uid === uid);
                    if (index !== -1) {
  ...
  tappedOnItems(event, user) {
    ...
    this.userListArray.map(item => {
      if (item.conversationWith.uid === user.uid) {
    ...
    new CometChat.UserListener({
      onUserOnline: onlineUser => {
        ...
        for (let i = 0; i < this.userListArray.length; i++) {
          if (this.userListArray[i].conversationWith.uid === onlineUser.uid) {
            this.userListArray[i].conversationWith.status = 'Online';
          }
        }
      ...
      console.log('On User Offline:', { offlineUser });
        for (let i = 0; i < this.userListArray.length; i++) {
          if (this.userListArray[i].conversationWith.uid === offlineUser.uid) {
            this.userListArray[i].conversationWith.status = 'Offline';
          }
        }
      }
    })
